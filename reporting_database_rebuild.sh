...
echo "Dump Core COLS Production Tables";
PGPASSWORD="$PRODUCTION_LEDGER_PASSWORD" pg_dump -t 'commercialoperator_*' --file /dbdumps/commercialoperator_core_prod.sql --format=custom --host $PRODUCTION_LEDGER_HOST --dbname $PRODUCTION_LEDGER_DATABASE --username $PRODUCTION_LEDGER_USERNAME
...
# IMPORT WLC 
echo "Importing COLS Core prod into reporting database";
PGSSLMODE=require PGPASSWORD="$TEMPORARY_LEDGER_PASSWORD"  pg_restore  --host=$TEMPORARY_LEDGER_HOST --dbname=$TEMPORARY_LEDGER_DATABASE --username=$TEMPORARY_LEDGER_USERNAME /dbdumps/commercialoperator_core_prod.sql

# GRANT SELECT to cols_ro account
for I in $(psql "host=$TEMPORARY_LEDGER_HOST port=5432 dbname=$TEMPORARY_LEDGER_DATABASE user=$TEMPORARY_LEDGER_USERNAME password=$TEMPORARY_LEDGER_PASSWORD sslmode=require" -c "SELECT tablename FROM pg_tables where tablename not like 'pg\_%' and tablename not like 'sql\_%' and tablename not like 'spatial\_%';" -t);
  do
  echo "GRANT SELECT ON $I TO cols_ro;";
  psql "host=$TEMPORARY_LEDGER_HOST port=5432 dbname=$TEMPORARY_LEDGER_DATABASE user=$TEMPORARY_LEDGER_USERNAME password=$TEMPORARY_LEDGER_PASSWORD sslmode=require" -c "GRANT SELECT ON $I TO cols_ro; " -t
done
...
rm /dbdumps/commercialoperator_core_prod.sql
