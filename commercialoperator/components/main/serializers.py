...
class ZoneSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = (
            ...
            "last_leaf",
            "visible",
            "allowed_zone_activities",
            "allowed_activities_ids",
        )
    ...
    def get_label(self, obj):
        """Pulled from parent (out serializer) --> ZoneSerializer"""
        return (
            "{} - {}".format(self.context.get("parent_name"), obj.name)
            if self.context.get("parent_id")
            else obj.name
        )
...
class LandActivityTabSerializer(serializers.Serializer):
    ...
    land_parks_external = ExternalRegionSerializer(
        many=True, read_only=True, source="regions"
    )
...
class EventsParkSerializer(serializers.ModelSerializer):
    ...
    def get_allowed_activities(self, obj):
        ...
        serializer = ActivityFilterSerializer(
