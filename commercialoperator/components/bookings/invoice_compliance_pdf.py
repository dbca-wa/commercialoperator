...
class Remittance(Flowable):
    ...
    def __footer_line(self):
        ...
        canvas.drawString((PAGE_WIDTH/4) * 2, current_y, currency(self.invoice.order.total_tax if not _is_gst_exempt(self.compliance, self.invoice) else 0.0))
...
def _create_header(canvas, doc, draw_page_number=True):
    ...
    canvas.drawString(current_x + invoice_details_offset, current_y - (SMALL_FONTSIZE + HEADER_SMALL_BUFFER) * 7, currency(invoice.order.total_tax if not _is_gst_exempt(compliance, invoice) else 0.0))
