...
class Booking(Payment):
    ...
    created_by = models.ForeignKey(
        ...
        blank=True,
        null=True,
        related_name="created_by_booking",
    )
...
class ParkBooking(RevisionedMixin):
    ...
    def __str__(self):
        return "Park {} : Arrival {} (Adults {}, Children {}, Free {})".format(
            self.park.name,
            self.arrival,
            self.no_adults,
            self.no_children,
            self.no_free_of_charge,
        )
    ...
    def as_line_items(self):
        ...
        lines = []
        if self.no_adults != 0 or (
            self.no_adults_same_tour is not None and self.no_adults_same_tour != 0
        ):
            if self.same_tour_group and self.no_adults_same_tour is not None:
                if self.no_adults_same_tour > 0:
                    lines.append(
                        add_line_item(
                            "Adult (Same Tour Group, Total {})".format(self.no_adults),
                            price=self.park.adult_price,
                            no_persons=self.no_adults_same_tour,
                            same_tour_group=self.same_tour_group,
                        )
                    )
                ...
                lines.append(
                    add_line_item(
                        "Adult", price=self.park.adult_price, no_persons=self.no_adults
                    )
                )

        if self.no_children != 0 or (
            self.no_children_same_tour is not None and self.no_children_same_tour != 0
        ):
            if self.same_tour_group and self.no_children_same_tour is not None:
                if self.no_children_same_tour > 0:
                    lines.append(
                        add_line_item(
                            "Child (Same Tour Group, Total {})".format(
                                self.no_children
                            ),
                            price=self.park.child_price,
                            no_persons=self.no_children_same_tour,
                            same_tour_group=self.same_tour_group,
                        )
                    )
        ...
        if self.no_free_of_charge != 0 or (
            self.no_free_of_charge_same_tour is not None
            and self.no_free_of_charge_same_tour != 0
        ):
            if self.same_tour_group and self.no_free_of_charge_same_tour is not None:
                if self.no_free_of_charge_same_tour > 0:
                    lines.append(
                        add_line_item(
                            "Free (Same Tour Group, Total {})".format(
                                self.no_free_of_charge
                            ),
                            price=0.0,
                            no_persons=self.no_free_of_charge_same_tour,
                            same_tour_group=self.same_tour_group,
                        )
                    )
                    ...
                    lines.append(
                        add_line_item(
                            ...
                            price=0.0,
                            no_persons=self.no_free_of_charge,
                            same_tour_group=self.same_tour_group,
                        )
                    )
            elif self.no_free_of_charge > 0:
                lines.append(
                    add_line_item("Free", price=0.0, no_persons=self.no_free_of_charge)
                )
...
class ApplicationFee(Payment):
    ...
    created_by = models.ForeignKey(
        ...
        blank=True,
        null=True,
        related_name="created_by_application_fee",
    )
...
class ComplianceFee(Payment):
    ...
    created_by = models.ForeignKey(
        ...
        blank=True,
        null=True,
        related_name="created_by_compliance_fee",
    )
...
class FilmingFee(Payment):
    ...
    created_by = models.ForeignKey(
        ...
        blank=True,
        null=True,
        related_name="created_by_filming_fee",
    )
    deferred_payment_date = models.DateField(blank=True, null=True)
    payment_due_notification_sent = models.BooleanField(default=False)
    ...
    def __str__(self):
        return "Proposal {} : Invoice {}".format(
            self.proposal, self.filming_fee_invoices.order_by("-id").first()
        )
