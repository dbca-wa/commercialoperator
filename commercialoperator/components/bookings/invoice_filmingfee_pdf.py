    ...
    def __footer_line(self):
        ...
        canvas.drawString(
            ...
            format_currency(
                ...
                if not _is_gst_exempt(self.proposal, self.invoice)
                else 0.0
            ),
        )
        canvas.drawString(
...
def _create_header(canvas, doc, draw_page_number=True):
    ...
    canvas.drawString(
        ...
        current_y - (SMALL_FONTSIZE + HEADER_SMALL_BUFFER) * 6,
        format_currency(
            invoice.order.total_tax if not _is_gst_exempt(proposal, invoice) else 0.0
        ),
    )
    canvas.drawRightString(
        current_x + 20,
        ...
        for item in items:
            ...
            data.append(
                [
                    ...
                    item["quantity"],
                    format_currency(item["price_incl_tax"]),
