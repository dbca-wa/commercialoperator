...
class Remittance(Flowable):
    ...
    def __footer_line(self):
        ...
        canvas.drawString((PAGE_WIDTH/4) * 2, current_y, currency(self.invoice.order.total_tax if not _is_gst_exempt(self.proposal, self.invoice) else 0.0))
...
def _create_header(canvas, doc, draw_page_number=True):
    ...
    canvas.drawString(current_x + invoice_details_offset, current_y - (SMALL_FONTSIZE + HEADER_SMALL_BUFFER) * 6, currency(invoice.order.total_tax if not _is_gst_exempt(proposal, invoice) else 0.0))
    ...
    if bi and bi[0].deferred_payment_date and invoice.payment_method in [invoice.PAYMENT_METHOD_BPAY]:
