...
class Approval(RevisionedMixin):
    ...
    submitter = models.ForeignKey(
        ...
        blank=True,
        null=True,
        related_name="commercialoperator_approvals",
    )
    ...
    def _notification_dates(self, _date):
        ...
        notification_months = (
            self.current_proposal.other_details.notification_months_tolist
        )
        if self.expiry_date:
            ...
            if preferred_licence_period == LicencePeriod.LICENCE_PERIOD_2_MONTHS:
                ...
                notification_dates = [
                    self.expiry_date - relativedelta(months=num_months)
                    for num_months in notification_months
    ...
    def is_renewable(self):
        ...
        return (
            self.current_proposal.application_type.name == ApplicationType.TCLASS
            and self.current_proposal.other_details.preferred_licence_period
            != LicencePeriod.LICENCE_PERIOD_2_MONTHS
        )
    ...
    def renew_months(self):
        ...
        if self.is_renewable:
            ...
            return LicencePeriod.objects.get(
                licence_period=preferred_licence_period
            ).renewal_month
        return None
    ...
    def renew_enable_date(self):
        ...
        """
        return (
            self.expiry_date - relativedelta(months=self.renew_months)
            if self.renew_months
            else None
        )
    ...
    def next_id(self):
        ...
        future_ids = [
            int(i.split("L")[1])
            for i in Approval.objects.filter(reserved_licence=True).values_list(
        ...
        ids = map(
            ...
            [
                i.split("L")[1]
                for i in Approval.objects.filter(reserved_licence=False).values_list(
                    "lodgement_number", flat=True
                )
                if i
            ],
    ...
    def can_extend(self):
        if self.current_proposal.application_type.name == "E Class":
            return (
                self.current_proposal.application_type.max_renewals > self.renewal_count
            )
        return False
    ...
    def can_amend(self):
        try:
            ...
            if self.current_proposal.is_lawful_authority:
                if (
                    self.current_proposal.is_lawful_authority_finalised
                    and self.can_renew
                ):
                    return True
                else:
    ...
    def can_reissue_lawful_authority(self):
        if (
            self.current_proposal.is_lawful_authority
            and self.current_proposal.is_lawful_authority_finalised
        ):
    ...
    def requirement_docs(self):
        if self.is_lawful_authority:
            approved_district_proposals_ids = (
                self.current_proposal.district_proposals.filter(
                    processing_status="approved"
                ).values_list("id", flat=True)
            )
            requirement_ids = (
                self.current_proposal.requirements.filter(
    ...
    def generate_doc(self, user, preview=False):
        from commercialoperator.components.approvals.pdf import (
            create_approval_doc,
...
reversion.register(
    Approval,
    follow=["compliances", "documents", "comms_logs", "action_logs", "notifications"],
)
reversion.register(
    ApprovalDocument,
    follow=["licence_document", "cover_letter_document", "renewal_document"],
)
