...
class ComplianceInternalNotificationOnlyEmail(TemplateEmailBase):
    ...
    html_template = (
        "commercialoperator/emails/send_internal_notification_only_email.html"
    )
...
def send_amendment_email_notification(
    ...
    submitter = (
        compliance.compliance_submitter_email
        if compliance.submitter and compliance.submitter.email
        else compliance.proposal.proposal_submitter_email
    )
    all_ccs = []
