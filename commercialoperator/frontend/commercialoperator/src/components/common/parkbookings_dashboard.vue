                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        mRender:function (data,type,full) {
                            let arrival_dates = '';
                            arrival_dates += (full.arrival != '' && full.arrival != null ? moment(full.arrival).format(vm.dateFormat): '')
    ...
    watch:{
        ...
        filterProposalStatus: function() {
            ...
            if (vm.filterProposalStatus!= 'All') {
                vm.$refs.proposal_datatable.vmDataTable.columns(9).search(vm.filterProposalStatus).draw();
            } else {
                vm.$refs.proposal_datatable.vmDataTable.columns(9).search('').draw();
            }
        },
        filterProposalPaymentMethod: function() {
            let vm = this;
            if (vm.filterProposalPaymentMethod!= 'All') {
                vm.$refs.proposal_datatable.vmDataTable.columns(10).search(vm.filterProposalPaymentMethod).draw();
            } else {
                vm.$refs.proposal_datatable.vmDataTable.columns(10).search('').draw();
            }
        },
