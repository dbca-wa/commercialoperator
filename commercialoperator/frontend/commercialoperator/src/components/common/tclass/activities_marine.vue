    ...
    methods: {
        ...
        get_park_activities: function () {
            ...
            for (var i = 0; i < vm.selected_zone_ids.length; i++) {
                ...
                if (park_id !== null) {
                    activities_by_zone.push({
                        park: park_id,
                        activities: vm.selected_activities,
                        access_point: '',
                    });
                    park_activities.push({
                        zone: zone_id,
                        activities: activities_by_zone,
                    });
                } else {
                    console.log(
                        'ERROR: Park ID not found for zone_id ' + zone_id
                    );
                }
            }
                        ...
                        if (vm.required_documents_list[j].park != null) {
                            if (
                                ...
                                if (
                                    ...
                                    marine_parks_activities[i].park
                                ) {
                            ...
                            for (
                                ...
                                var k = 0;
                                k <
                                marine_parks_activities[i].activities.length;
                                k++
                            ) {
                                for (
                                    // eslint-disable-next-line no-redeclare
        ...
        checkAllowedActivities: function () {
            ...
            if (parks_list.length > 0) {
                ...
                vm.park_error_list.push(
                    'Notice: You have not selected any activity that is permitted within the following park(s): ' +
                        unique_park_list +
                        '. Click the park name to view and edit the permitted activities.'
                );
            }
        },
        edit_activities: function (node) {
            ...
            var activities =
                vm.marine_parks_activities[park_idx].activities[zone_idx]
                    .activities;
            var access_point =
                vm.marine_parks_activities[park_idx].activities[zone_idx]
                    .access_point;
            ...
            this.$refs.edit_activities.new_activities =
        ...
        refreshSelectionFromResponse: function (
            ...
            vm.marine_parks_activities[park_idx].activities.push(
                new_activities
            );
            ...
            for (var i = 0; i < parks.length; i++) {
                if (vm.is_external && !parks[i].park.visible_to_external) {
                    ...
                    for (var j = 0; j < parks[i].zones.length; j++) {
                        ...
                        for (
                            var k = 0;
                            k < parks[i].zones[j].park_activities.length;
                            k++
                        ) {
                            park_activities.push(
                                parks[i].zones[j].park_activities[k].activity
                            );
                        }
