...
export default {
    ...
    computed: {
        ...
        delete_url: function() {
            return (this.park_id) ? '/api/proposal_pre_event_parks/'+this.park_id+'/delete_document/' : '';
        }
    },
    methods:{
        ...
        fetchPark: function(vid){
            let vm=this;
            Vue.http.get(helpers.add_endpoint_json(api_endpoints.proposal_pre_event_parks,vid)).then((res) => {
                      vm.park=res.body; 
                      if(vm.park.park)
                      {
        ...
        sendData:function(){
            ...
            {
                vm.$http.post(api_endpoints.proposal_pre_event_parks,formData,{
                        emulateJSON:true,
                    }).then((response)=>{
            ...
            vm.$http.post(helpers.add_endpoint_json(api_endpoints.proposal_pre_event_parks,vm.park_id+'/edit_park'),formData,{
                        emulateJSON:true,
                    }).then((response)=>{
                        vm.issuingPark = false;
                        vm.park={};
   ...
   mounted:function () {
        ...
        vm.park.pre_event_park_documents = vm.$refs.filefield.uploaded_documents;
   }
}
</script>
